...
- Dashboard with stats, auto-overdue marking, compliance sync, and compact mini charts (Orders, Revenue, Profit, Customers) in a single row below date range — powered by Recharts with gradient area fills, custom tooltips, cumulative totals shown as header values. Charts respect selected date range; refresh only on page load or Refresh Database button. API: `GET /api/dashboard/charts`
...
- Customer Portal Links (one link per customer, auto-generated on invoice creation)
  ...
  - Unread message alerts on order cards: Both admin and customer portal show red pulsing badges with MessageSquare icon and unread count on each order card. Admin uses `GET /api/chats/unread-counts` (react-query, 15s refetchInterval). Customer portal uses `GET /api/portal/:token/chats/unread-counts` (manual fetch + setInterval 15s). Storage method `getUnreadChatCountsForCustomer(customerId)` filters by customer's orders only.
