...
- Customer Portal Links (one link per customer, auto-generated on invoice creation)
  ...
  - Admin orders list unread alerts: `GET /api/chats/unread-counts` returns `{orderId: count}` for unread customer messages. Orders page shows red pulsing badge with MessageSquare icon and count on each order card that has unread messages, auto-refreshes every 15 seconds.
